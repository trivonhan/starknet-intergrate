...
export class StarknetService {
  async main(): Promise<void> {
    ...
    const compiledOZAccount = json.parse(
      fs
        .readFileSync(__dirname + '/contracts/OZAccount.json')
        .toString('ascii'),
    );
    ...
    const compiledErc20 = json.parse(
      fs.readFileSync(__dirname + '/contracts/ERC20.json').toString('ascii'),
    );

    const recipient = process.env.RECIPIENT;
    console.log('Recipient address ', recipient);
    const constructorData = [
      encodeShortString('MadTikToken'),
      encodeShortString('MTT'),
      18,
      1000000,
      0,
      recipient, // Here the `recipient` receives the initial 1000 tokens
    ];
    console.log('Deploying ERC20 Contract...', constructorData);
    ...
    console.log('ERC20 address ', erc20Response.contract_address);
    // Wait for the deployment transaction to be accepted on StarkNet
    console.log(
